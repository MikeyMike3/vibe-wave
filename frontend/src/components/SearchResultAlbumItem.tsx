...
export const SearchResultAlbumItem = ({ album }: SearchResultAlbumItemProps) => {
  ...
  return (
    <Link
      to={`/album/${album.id}`}
      className="group relative z-50 inline-block flex-shrink-0 rounded-xl p-4 duration-150 hover:cursor-grab lg:hover:bg-bgAccent"
    >
      <div
      ...
      <p className="my-2 line-clamp-1 text-textPrimary lg:group-hover:text-aqua">{album?.name}</p>
      <p className="line-clamp-1 text-textAccent lg:group-hover:text-[#00CCCC]">
